...
public class ParsingMap {
	...
	private void buildMap() {
		Scanner s = null;
		try {
			s = new Scanner(new File(CoreConstants.MAP_PARSING_FILENAME));
		} catch (FileNotFoundException e) {
			System.out.println(CoreConstants.MAP_PARSING_ERROR_STRING + e.getLocalizedMessage());
			System.exit(CoreConstants.MAP_PARSING_ERROR_CODE);
		}
		while (s.hasNextLine()) {
			String line = s.nextLine();
			int pivot = line.indexOf(' ');
			if (pivot == -1) {
				System.out.println(CoreConstants.MAP_PARSING_ERROR_INCORRECT_FORMAT);
				System.exit(CoreConstants.MAP_PARSING_ERROR_CODE);
			}
			...
			try {
				...
				System.out.println(CoreConstants.MAP_PARSING_ERROR_NO_CLASS + e.getLocalizedMessage());
				System.exit(CoreConstants.MAP_PARSING_ERROR_CODE);
			}
